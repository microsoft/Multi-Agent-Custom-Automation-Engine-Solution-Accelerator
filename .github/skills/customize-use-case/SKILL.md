...
For GA/demo-ready use cases, make activation deterministic:

- Use a stable, explicit `team_id`; deployment handoff must upload with that same ID instead of relying on backend-generated IDs.
- Include 3-4 starting tasks with short `name` values and full `prompt` values so the Home page renders useful Quick task cards after selection.
- For every RAG agent, define one explicit dataset-to-index mapping and document the intended Search index name beside the dataset path.
- Do not instruct users to rerun packaged sample-data scripts for custom use cases unless those scripts have been intentionally updated for the new pack. Prefer scoped upload/index commands for the exact files generated by the skill.
- Add an expected smoke-test contract to the handoff: `/api/v4/init_team` returns the generated `team_id`, expected agent count, and non-empty `starting_tasks`; the UI shows the selected team and Quick task cards.
- Leave Azure networking, Cosmos firewall, Search, Blob, and App Service setting mutations to `deploy-adaptation`; generated docs should name requirements, not perform cloud changes.
