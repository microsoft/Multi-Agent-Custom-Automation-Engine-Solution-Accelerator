...
sys.modules['models.messages'] = Mock(
    ...
    PlanApprovalResponse=MockPlanApprovalResponse,
...
messages = sys.modules['models.messages']
...
class TestHumanApprovalMagenticManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_plan_task_ledger_none(self):
        ...
        with patch.object(self.manager, 'plan', wraps=self.manager.plan):
            with patch('backend.orchestration.human_approval_manager.StandardMagenticManager.plan') as mock_super_plan:
    ...
    async def test_wait_for_user_approval_success(self):
        ...
        with patch('backend.orchestration.human_approval_manager.messages.PlanApprovalResponse', MockPlanApprovalResponse):
    ...
    async def test_replan_with_none_message(self):
        ...
        with patch('backend.orchestration.human_approval_manager.StandardMagenticManager.replan', return_value=None):
            result = await self.manager.replan(self.test_context)
