...
class AgentRegistry:
    ...
    def register_agent(self, agent: Any, user_id: Optional[str] = None) -> None:
        ...
        with self._lock:
            try:
                ...
                self.logger.info(
                    "Registered agent: %s (ID: %s, User: %s)",
                    type(agent).__name__,
                    agent_id,
                    user_id,
                )
            except Exception as e:
                self.logger.error("Failed to register agent: %s", e)
    ...
    def unregister_agent(self, agent: Any) -> None:
        ...
        with self._lock:
            try:
                ...
                self.logger.error("Failed to unregister agent: %s", e)
    ...
    async def cleanup_all_agents(self) -> None:
        ...
        self.logger.info("Starting cleanup of %d total agents", len(all_agents))
        ...
        for agent in all_agents:
            if hasattr(agent, "close"):
                ...
                agent_name = getattr(agent, "agent_name", getattr(agent, "name", type(agent).__name__))
                self.logger.warning(
                    "Agent %s has no close() method — just unregistering", agent_name
                )
                self.unregister_agent(agent)

        if cleanup_tasks:
            self.logger.info("Executing %d cleanup tasks...", len(cleanup_tasks))
            results = await asyncio.gather(*cleanup_tasks, return_exceptions=True)

            success_count = 0
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    self.logger.error("Error cleaning up agent %d: %s", i, result)
                else:
                    success_count += 1

            self.logger.info(
                "Successfully cleaned up %d/%d agents", success_count, len(cleanup_tasks)
            )
    ...
    async def _safe_close_agent(self, agent: Any) -> None:
        ...
        try:
            agent_name = getattr(agent, "agent_name", getattr(agent, "name", type(agent).__name__))
            self.logger.info("Closing agent: %s", agent_name)
            ...
            self.logger.info("Successfully closed agent: %s", agent_name)

        except Exception as e:
            agent_name = getattr(agent, "agent_name", getattr(agent, "name", type(agent).__name__))
            self.logger.error("Failed to close agent %s: %s", agent_name, e)
